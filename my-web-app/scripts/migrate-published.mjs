/**
 * migrate-published.mjs
 * อัปเดต isPublished ใน Firestore collection "ipphone_directory"
 *
 * วิธีใช้:
 *   1. ดาวน์โหลด Service Account JSON จาก Firebase Console
 *      → Project Settings → Service accounts → Generate new private key
 *   2. วางไฟล์ที่: scripts/serviceAccount.json
 *   3. รัน: node scripts/migrate-published.mjs
 */
import { createRequire } from 'module'
const require = createRequire(import.meta.url)

import { readFileSync } from 'fs'
import { fileURLToPath } from 'url'
import { dirname, join } from 'path'

const __dirname = dirname(fileURLToPath(import.meta.url))
const serviceAccount = JSON.parse(readFileSync(join(__dirname, 'serviceAccount.json'), 'utf8'))

const admin = require('firebase-admin')
admin.initializeApp({ credential: admin.credential.cert(serviceAccount) })
const db = admin.firestore()

const PUBLISHED_EXTENSIONS = new Set([
  '71208', '71209', '71313', '71101', '97086',
  '70233', '70215', '97280', '70214', '79917',
  '70218', '97283', '70226', '71812', '97267',
  '98107', '97273', '79932', '79933', '70735',
  '70724', '97277', '70706',
  '70305', '70307', '70308', '70315', '70312',
  '70313', '70316', '70310', '70311', '70309',
  '70314', '97774', '97754',
  '70321', '70323', '70319', '70324', '70326',
  '97780', '97715', '70302', '97744', '97782',
  '97750', '97363',
  '70819',
  '71416', '97155', '71415', '71417', '71418', '97477',
  '71508', '98041', '71504', '97460', '97461',
  '97628', '97630', '79929', '98042',
  '70502', '70503', '97160',
  '70905', '70904', '70903', '70956', '97165',
  '70961', '70978', '70909', '70957', '97166',
  '98028', '70974', '97315', '70908', '70940',
  '70916', '70921', '98021', '70918', '70917',
  '70954', '70902', '70945', '97193', '97405',
  '70949', '70948', '70947', '70972', '97200',
  '70931', '70927', '70929', '70928', '70930',
  '98026', '70975', '98027', '70980', '70914',
  '97201', '70913', '70910', '97282',
  '70424',
  '72101', '72102', '97394',
  '97177', '97195', '97356', '97179', '97257', '97406',
  '71038', '71051', '97176', '97189', '97064', '97234',
  '98043', '97100', '97106',
  '70129', '70118', '70126', '97104', '70130',
  '71711',
])

async function migrate() {
  console.log('📡 กำลังโหลดข้อมูลจาก Firestore...')
  const snap = await db.collection('ipphone_directory').get()
  console.log(`📋 พบ ${snap.size} รายการ`)

  let published = 0
  let hidden = 0
  let batchCount = 0
  const BATCH_SIZE = 400

  let batch = db.batch()

  for (const document of snap.docs) {
    const ext = document.data().ipPhoneNumber ?? ''
    const shouldPublish = PUBLISHED_EXTENSIONS.has(ext)
    batch.update(document.ref, { isPublished: shouldPublish })
    if (shouldPublish) { published++ } else { hidden++ }
    batchCount++

    if (batchCount % BATCH_SIZE === 0) {
      await batch.commit()
      console.log(`  ✅ commit ${batchCount} รายการแล้ว...`)
      batch = db.batch()
    }
  }

  if (batchCount % BATCH_SIZE !== 0) {
    await batch.commit()
  }

  console.log(`\n✅ Migration สำเร็จ!`)
  console.log(`   เผยแพร่: ${published} รายการ`)
  console.log(`   ซ่อน:    ${hidden} รายการ`)
  process.exit(0)
}

migrate().catch((err) => {
  console.error('❌ Error:', err.message)
  process.exit(1)
})
