...
import api from '@/services/api' // <--- เรียกใช้ Axios ของเรา
import axios from 'axios' // <--- ใช้สำหรับ Type Guard
...
// ⚠️ ชั่วคราว: ระบบ Google Login
const handleGoogleLogin = (): void => {
  // เนื่องจากเราเพิ่งย้ายมา .NET ระบบ Google OAuth จะต้องทำที่ฝั่ง Backend ใน Phase ถัดๆ ไป
  toast.warn('ระบบเข้าสู่ระบบด้วย Google กำลังอยู่ระหว่างการปรับปรุงมาใช้เซิร์ฟเวอร์ใหม่', 'แจ้งเตือน')
}
</script>
...
<template>
  <div class="min-h-screen flex">
    <div
      ...
      <div class="absolute top-0 right-0 w-96 h-96 bg-indigo-600/10 rounded-full -translate-y-1/2 translate-x-1/2">
      </div>
      <div class="absolute bottom-0 left-0 w-64 h-64 bg-indigo-800/20 rounded-full translate-y-1/2 -translate-x-1/2">
      </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          <div class="mb-7">
            ...
            <p class="text-gray-400 text-sm mt-1">
              {{ isLoginMode ? 'ยินดีต้อนรับกลับ กรุณาล็อกอินเพื่อดำเนินการต่อ' :
          ...
          <div class="flex flex-col gap-4">
            ...
            <form @submit.prevent="handleEmailAuth" class="flex flex-col gap-4">
              <div v-if="!isLoginMode" class="flex flex-col gap-1.5">
                <label class="text-sm font-semibold text-gray-700">ชื่อ-นามสกุล <span
              ...
              <div v-if="!isLoginMode" class="flex flex-col gap-1.5">
                ...
                <Select v-model="selectedDepartmentId" :options="departments" optionLabel="name" optionValue="id"
                  placeholder="เลือกหน่วยงานของคุณ" class="w-full" />
              </div>
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <Password v-model="password" :feedback="!isLoginMode" toggleMask placeholder="••••••••"
                  inputClass="w-full" class="w-full" />
              </div>

              <Button type="submit" :label="isLoginMode ? 'เข้าสู่ระบบ' : 'สมัครสมาชิก'"
                :icon="isLoginMode ? 'pi pi-sign-in' : 'pi pi-user-plus'" class="w-full font-bold mt-1"
                :loading="isLoadingEmail" :disabled="isLoadingGoogle" />
            </form>

            <p class="text-center text-sm text-gray-500">
              {{ isLoginMode ? 'ยังไม่มีบัญชี?' : 'มีบัญชีแล้ว?' }}
              <button @click="isLoginMode = !isLoginMode; errorMessage = ''; successMessage = '';"
            ...
            <Button type="button" label="เข้าสู่ระบบด้วย Google" icon="pi pi-google" severity="secondary" outlined
              class="w-full font-semibold" @click="handleGoogleLogin" :loading="isLoadingGoogle"
